...
#     # ----- 性能分析代码开始 -----
#     if should_print and timings is not None:
...
#     n: int = invalid_attn_mask.size(-1)
#     if delta_x_offsets is not None:
#         # ... (这部分是cache逻辑，暂时不作为分析重点)
#         padded_q, padded_k = cached_q, cached_k
#         # ... 
#     else:
#         # 步骤 1: 计时 q, k 从 jagged 到 dense 的转换
#         padded_q = jagged_to_padded_dense_py(values=q, offsets=[x_offsets], max_lengths=[n], padding_value=0.0)
#         padded_k = jagged_to_padded_dense_py(values=k, offsets=[x_offsets], max_lengths=[n], padding_value=0.0)

#     if should_print and timings is not None:
#         if torch.cuda.is_available():
#             torch.cuda.synchronize()
#         timings['C_Attn_1_jagged_to_padded_qk'] = time.perf_counter() - step_start_time
#         step_start_time = time.perf_counter()
        
#     # 步骤 2: 计时 qk 的 einsum 计算
#     qk_attn = torch.einsum("bnhd,bmhd->bhnm", padded_q.view(B, n, num_heads, attention_dim), padded_k.view(B, n, num_heads, attention_dim))

#     if should_print and timings is not None:
#         if torch.cuda.is_available():
#             torch.cuda.synchronize()
#         timings['C_Attn_2_einsum_qk'] = time.perf_counter() - step_start_time
#         step_start_time = time.perf_counter()

#     # (相对位置偏置等操作耗时通常很小，暂不单独计时)
#     if all_timestamps is not None:
#         qk_attn = qk_attn + rel_attn_bias(all_timestamps).unsqueeze(1)
#     qk_attn = F.silu(qk_attn) / n
#     qk_attn = qk_attn * invalid_attn_mask.unsqueeze(0).unsqueeze(0)

#     # 步骤 3: 计时 v 从 jagged 到 dense 的转换
#     padded_v = jagged_to_padded_dense_py(v, [x_offsets], [n]).reshape(B, n, num_heads, linear_dim)
    
#     if should_print and timings is not None:
#         if torch.cuda.is_available():
#             torch.cuda.synchronize()
#         timings['C_Attn_3_jagged_to_padded_v'] = time.perf_counter() - step_start_time
#         step_start_time = time.perf_counter()
        
#     # 步骤 4: 计时 attn_output 的 einsum 计算
#     attn_output_padded = torch.einsum("bhnm,bmhd->bnhd", qk_attn, padded_v).reshape(B, n, num_heads * linear_dim)

#     if should_print and timings is not None:
#         if torch.cuda.is_available():
#             torch.cuda.synchronize()
#         timings['C_Attn_4_einsum_attn_out'] = time.perf_counter() - step_start_time
#         step_start_time = time.perf_counter()
        
#     # 步骤 5: 计时最终结果从 dense 到 jagged 的转换
#     attn_output = dense_to_jagged_py(attn_output_padded, [x_offsets])[0]

#     if should_print and timings is not None:
#         if torch.cuda.is_available():
#             torch.cuda.synchronize()
#         timings['C_Attn_5_dense_to_jagged'] = time.perf_counter() - step_start_time
#         step_start_time = time.perf_counter()
...
#     def forward(self, x, x_offsets, all_timestamps, invalid_attn_mask, delta_x_offsets=None, cache=None, return_cache_states=False):
        
#         # ----- 性能分析代码开始 -----
#         self.forward_counter += 1
#         # 每20次 forward 调用打印一次，可以根据 num_blocks 调整
#         should_print = self.forward_counter % 20 == 0
#         if should_print:
#             if torch.cuda.is_available():
#                 torch.cuda.synchronize()
#             timings = {}
...
#             if torch.cuda.is_available():
#                 torch.cuda.synchronize()
#             timings['A_Norm_Input'] = time.perf_counter() - step_start_time
#             step_start_time = time.perf_counter()
...
#             if self._linear_activation == "silu":
#                 batched_mm_output = F.silu(batched_mm_output)
#             u, v, q, k = torch.split(batched_mm_output, [self._linear_dim * self._num_heads, self._linear_dim * self._num_heads, self._attention_dim * self._num_heads, self._attention_dim * self._num_heads], dim=1)
#         else:
#             raise ValueError(f"Unknown self._linear_config {self._linear_config}")
...
#             if torch.cuda.is_available():
#                 torch.cuda.synchronize()
#             timings['B_Linear_Projection_UVQK'] = time.perf_counter() - step_start_time
#             step_start_time = time.perf_counter()
...
#                 rel_attn_bias=self._rel_attn_bias,
#                 # 把这两个参数传进去
#                 timings=timings if should_print else None,
#                 should_print=should_print,
#             )
#         elif self._normalization == "softmax_rel_bias":
#             # ... (这部分代码为了简洁省略，计时逻辑和上面一样)
#             raise NotImplementedError("Softmax profiling not added for brevity")
#         else:
#             raise ValueError(f"Unknown normalization method {self._normalization}")
...
#             if torch.cuda.is_available():
#                 torch.cuda.synchronize()
#             # 这一步包含了所有可疑的 _py 函数调用，是最大嫌疑
#             timings['C_Attention_Calculation'] = time.perf_counter() - step_start_time
#             step_start_time = time.perf_counter()
...
#         block_output = (self._o(F.dropout(o_input, p=self._dropout_ratio, training=self.training)) + x)
#         new_outputs = block_output
...
#             if torch.cuda.is_available():
#                 torch.cuda.synchronize()
#             timings['D_Output_Projection'] = time.perf_counter() - step_start_time
#             timings['E_Total_Block_Time'] = time.perf_counter() - start_time
#             print(f"\n--- Profiling HSTU Block (call #{self.forward_counter}) ---")
