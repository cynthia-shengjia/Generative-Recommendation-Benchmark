...
from tools.sdpo_trainer_utils import setup_training
import random
import numpy as np
...
def stage2_train_generation_model(model_config, rqvae_config, output_dirs, accelerator, logger, force_retrain=False):
    ...
    try:
        ...
        train_dataset = SeqModelTrainingDataset(
            ...
            tokenizer=tokenizer, config=model_config, mode='offline-rl'
        )
        ...
        # 🔴 关键修改：为 evaluation 创建标准的 data collator
        eval_data_collator = TrainSeqRecDataCollator(
        ...
        trainer = setup_training(
            ...
            eval_data_collator=eval_data_collator,  # ✅ 新增参数
        )
